...
├── .DS_Store
├── .env.example                        # Stores email address and password (to be renamed to .env)
├── .gitignore
├── .ipynb_checkpoints/
│   └── Phishcord-checkpoint.ipynb
├── app.py                              # Flask backend server
├── audio.mp3                           # Alert sound for detections
├── background.js                       # Chrome extension - background script
├── current_model.txt                   # Stores model that's selected
├── icon.png                            # Extension icon
├── imap_listener.py                    # Script to connect and read unseen email via IMAP 
...
│   └── sound.png
├── svc_model.pkl
├── templates/                          # Placeholder to test if server is running
│   └── index.html
├── vector.pkl                          # TF-IDF vectoriser
└── xg_model.pkl                        # XGBoost model
```
